...
> 信任记录按“钩子定义哈希”绑定：插件更新（hooks.json 或版本变化）后旧信任会失效，
> 需要重新信任一次。信任后桌面应用与 CLI 都会强制注入，开新线程即可生效。
...
- **主通道（钩子）**：SessionStart/UserPromptSubmit 钩子把契约作为 developer
  上下文强制注入，桌面应用与 CLI 都支持；前提是信任钩子（见安装警告）。
- **兜底通道（技能）**：SKILL.md 是自执行契约，始终加载；会话里没有钩子记录时
  （例如尚未信任或某构建不支持），模型仍会按契约工作，并主动说明当前走的是技能通道。
...
有新会话的 `SessionStart` / `UserPromptSubmit` 记录就是钩子通道生效；没有记录但模型
按契约工作，就是技能通道在兜底——两种都算“开了”，模型会主动说明当前走的是哪条通道。
...
hooks/hooks.json              三个生命周期钩子（SessionStart / UserPromptSubmit / SessionEnd）；SessionStart 匹配 compact，压缩后按已锁定模式重新锚定
