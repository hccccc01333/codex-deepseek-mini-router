...
| 根插件清单 `.codex-plugin/plugin.json` | ✅ 0.4.0 |
...
Compress-Archive -Path "$src\*" -DestinationPath "$env:TEMP\deepseek-mini-router-0.4.0.zip"
```

ZIP 根目录必须包含：

- `.codex-plugin/plugin.json`（0.4.0，displayName = DeepSeek Mini-Router）
